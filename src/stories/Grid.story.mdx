...
The Grid is a flexible 12 column layout, built using CSS Grid but with fallback to Flexbox. It
consists of the `<Grid>` with a collection of `<GridItem>` elements.
...
<Preview>
  <Story name="Theme grid">
    <ThemeProvider theme={{ grids: { grid: { gridGap: 24 } } }}>
      <Grid>
        <GridItem col={12}>
          <Item>col=12</Item>
        </GridItem>
...
[spacer gif](https://en.wikipedia.org/wiki/Spacer_GIF). Set the amount of `col`'s it should offset
the next items. If you set the `col` size to `0` this will cause the `<GridItem>` to render with
`display: none;`, effectively removing the item from the layout.
...
<Preview>
  <Story name="Spacer item">
    <Grid gridGap="24px">
      ...
      <GridItem col={4} sx={{ bg: 'antiquewhite' }} />
      <GridItem col={4}>
        <Item>Item</Item>
      </GridItem>
      <GridItem col={[0, 1, 2]} sx={{ bg: 'antiquewhite' }} />
      <GridItem col={4}>
        <Item>Item</Item>
      </GridItem>
    </Grid>
...
when nesting grids, or to have fewer columns on smaller devices.
...
can stay on a sane value. Usually you won't need to use more than 4 columns on a mobile, or 8
columns on a tablet device. Because we need to support the flexbox fallback method, the responsive
handling is simplified a bit - Only the responsive array is supported.

It's important that the size of a `col` is never larger than the current `gridColumns`. Therefor a
warning will also be logged if this occurs during development, since you most likely forgot to
adjust for it.
...
<Props of={GridItem} exclude={['gridColumns', ...excludedProps]} />

#### Variants

The `<GridItem>` has the default `variant` value `grids.gridItem` - you can use it to modify the
default styling.
