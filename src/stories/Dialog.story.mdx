...
import { Dialog, DialogContent, DialogBackdrop, Box, Text, Heading } from '../index';
...
const DialogExample = () => {
  ...
  return (
    <Dialog isOpen={isOpen} onDismiss={onDismiss}>
      <DialogBackdrop />
      <DialogContent aria-labelledby="dialog-header" sx={{ bg: 'white', p: 3, borderRadius: 4 }}>
  ...
  <OpenState>
    {({ isOpen, onDismiss }) => (
      ...
      >
        ...
        <DialogContent aria-labelledby="dialog-header" sx={{ bg: 'white', p: 3, borderRadius: 4 }}>
          <Heading id="dialog-header">Example dialog</Heading>
          <Text my={3}>The dialog can present content as needed here.</Text>
          <button onClick={onDismiss}>Ok</button>
        </DialogContent>
      </Dialog>
    )}
...
<Props of={Dialog} exclude={allBoxExcludedProps} />

The component also inherits the [Box](/?path=/docs/components-box--page#props) props.
...
The component inherits the [Box](/?path=/docs/components-box--page#props) props. It currently
...
<Props
  of={DialogContent}
  exclude={allBoxExcludedProps.filter(prop => {
